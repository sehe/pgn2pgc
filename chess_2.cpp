...
char ChessSquare::pieceToChar() const
{
	switch(fContents)
	{
		...
		case empty: return ' ';
		default: assert(0); return 'x'; // notReached
	}
}
...
// position : to move : castleing : (50*2) - plys till draw : current move number
...
bool Board::processFEN(const string& FENPosition)
{
	...
	AdoptArray<char> adopter(FEN); // so that if return prematurely resource will be released by destructor
	CHECK_POINTER(FEN);
	...
	for(i = ranks() - 1; i >= 0; --i)
	{
		for(size_t j = 0; j < files(); ++j)
		{
			...
			if(!numEmpty) // seperate from the other if so that when numEmpty becomes zero again this will be executed
			{
	...
	switch(tolower(curToken[index])) // specs say only lower case char, but no need to be picky here
	{
...
bool Board::move(const ChessMove& move)
{
	...
	switch(move.type())
	{
		case ChessMove::whiteEnPassant:
			fBoard[move.rt() - 1][move.ft()] = ChessSquare(ChessSquare::empty); break;
		...
		case ChessMove::promoQueen:
			...
			fBoard[move.rt()][move.ft()] = fBoard[move.rt()][move.ft()].isWhite() ? ChessSquare(ChessSquare::whiteQueen) : ChessSquare(ChessSquare::blackQueen);
			break;
		case ChessMove::promoKnight:
			fBoard[move.rt()][move.ft()] = fBoard[move.rt()][move.ft()].isWhite() ? ChessSquare(ChessSquare::whiteKnight) : ChessSquare(ChessSquare::blackKnight);
			break;
		case ChessMove::promoRook:
			fBoard[move.rt()][move.ft()] = fBoard[move.rt()][move.ft()].isWhite() ? ChessSquare(ChessSquare::whiteRook) : ChessSquare(ChessSquare::blackRook);
			break;
		case ChessMove::promoBishop:
			fBoard[move.rt()][move.ft()] = fBoard[move.rt()][move.ft()].isWhite() ? ChessSquare(ChessSquare::whiteBishop) : ChessSquare(ChessSquare::blackBishop);
			break;
		case ChessMove::promoKing:
			fBoard[move.rt()][move.ft()] = fBoard[move.rt()][move.ft()].isWhite() ? ChessSquare(ChessSquare::whiteKing) : ChessSquare(ChessSquare::blackKing);
			break;
...
inline
void Board::genLegalMoves(List<ChessMove>* moves)
{
	assert(moves);
...
bool Board::processMove(const ChessMove& m, List<ChessMove>& allMoves)
{
	...
	unsigned plysSince = !fBoard[m.rf()][m.ff()].isPawn() && fBoard[m.rt()][m.ft()].isEmpty() ? fPlysSince + 1 : 0;
...
//inline
void Board::moveToAlgebraic(string* SAN, const ChessMove& move, List<ChessMove>& allMoves)
{
	assert(SAN);
...
//inline
bool Board::algebraicToMove(ChessMove* move, const char SAN[], List<ChessMove>& allMoves)
{
	assert(move);
	return AlgebraicToMove(SAN, *this, allMoves, move);
}

void Board::genLegalMoveSet(List<ChessMove>* allMoves, PriorityQueue<ChessMoveSAN>* allSAN)
{
	assert(allMoves);
	assert(allSAN);
...
inline
void Board::addMove(int rf, int ff, int rt, int ft, ChessMove::E_type type, List<ChessMove>* moves, PriorityQueue<ChessMoveSAN>* allSAN)
{
	string sanValue;
...
void Board::genLegalMoves(List<ChessMove>* allMoves, PriorityQueue<ChessMoveSAN>* allSAN)
{
	assert(allMoves);
	assert(allSAN);
				...
				if(fBoard[0][i] == ChessSquare::whiteKing)
				{
					...
					if(getCastle() & whiteQS)
					{
						...
						if(!pieceInWay && fBoard[0][0] == ChessSquare::whiteRook &&
							i - 2 > 0)
						{
							if(!WillBeInCheck(*this, ChessMove(0, i, 0, i - 1))) // cannot castle through check, or when in check
								addMove(0, i, 0, i - 2, ChessMove::whiteCastleQS, allMoves, allSAN);
						}
					}
					break; //!!? break is possible here, unless two kings on rank and one can castle (some form of wild?)
				}
			}
				...
				if(fBoard[ranks() - 1][i] == ChessSquare::blackKing)
				{
					if(getCastle() & blackKS)
					{
							...
							i + 2 < files())
						{
							...
							{
								addMove(ranks() - 1, i, ranks() - 1, i + 2, ChessMove::blackCastleKS, allMoves, allSAN);
							}
						}
					}
					if(getCastle() & blackQS)
					{
						...
						if(!pieceInWay && fBoard[ranks() - 1][0] == ChessSquare::blackRook &&
							i - 2 > 0)
						{
							if(!WillBeInCheck(*this, ChessMove(ranks() - 1, i, ranks() - 1, i - 1)))
								addMove(ranks() - 1, i, ranks() - 1, i - 2, ChessMove::blackCastleQS, allMoves, allSAN);
						}
					}
					break;
				}
...
void Board::removeIllegalMoves(List<ChessMove>* allMoves, PriorityQueue<ChessMoveSAN>* allSAN)
{
	bool isFound = false;
			...
			b.fBoard[m.rt()][m.ft()] == ChessSquare::blackKing)
		{
			if(b.canCaptureSquare(m.rt(), m.ft()))
			{
				...
				for(unsigned j = 0; j < allSAN->size(); ++j)
					if(allSAN->operator[](j).ChessMove::operator==(m)) // c++ at it's finest :)
					{
						allSAN->remove(j);
						break;
					}
...
void Board::moveToAlgebraicAmbiguity(string* out, const ChessMove& m) // doesn't worry about any ambiguities, nor does it indicate check or checkmate status (which don't alter sort order anyway)
{
	strstream o;
	...
	switch(fBoard[m.rf()][m.ff()].contents())
	{
		...
		case ChessSquare::blackKing:
			// castling
			if (m.rf() == m.rt() &&
					m.rf() == ((fBoard[m.rf()][m.ff()].isWhite()) ? 0 : ranks() - 1))
			{
...
void Board::genPseudoLegalMoves(List<ChessMove>* moves, PriorityQueue<ChessMoveSAN>* allSAN)
{
	assert(moves);
	assert(allSAN);
...
//							addMove(rf, ff, rf + 1, ff, ChessMove::promoKing, moves, allSAN); // some wild variants
...
//								addMove(rf, ff, rf + 1, ff + s, ChessMove::promoKing, moves, allSAN);
						...
						if (rf == ranks() - 4)
						{
							if (ff + s >= 0 && ff + s <= 7 && (enPassant() == ff + s || enPassant() == Board::allCaptures) &&
								fBoard[ranks() - 4][ ff + s].contents() == ChessSquare::blackPawn && fBoard[ranks() - 3][ff + s].isEmpty())
							{
...
//							addMove(rf, ff, rf - 1, ff, ChessMove::promoKing, moves, allSAN);
					...
					if (rf == ranks() - 2 && fBoard[ranks() - 3][ff].isEmpty() && fBoard[ranks() - 4][ff].isEmpty())
					{
...
//								addMove(rf, ff, rf - 1, ff + s, ChessMove::promoKing, moves, allSAN);
						...
						{
							if (ff + s >= 0 && ff + s <= 7 && (enPassant() == ff + s || enPassant() == Board::allCaptures) &&
								fBoard[3][ff + s].contents() == ChessSquare::whitePawn && fBoard[2][ff + s].isEmpty())
							{
...
bool Board::canCaptureSquare(unsigned targetRank, unsigned targetFile)
{
...
void Board::disambiguate(const ChessMove& move, string* san, PriorityQueue<ChessMoveSAN>& allSAN)
{
	if(san->length())
	{
		bool conflict = false, rankConflict = false, fileConflict = false;
		for(int i = 0; i < allSAN.size(); ++i)
		{
			if(allSAN[i].rt() == move.rt() && allSAN[i].ft() == move.ft() && // the same 'to' square
				allSAN[i].move() != move && // not the same move
				fBoard[move.rf()][move.ff()].contents() == fBoard[allSAN[i].rf()][allSAN[i].ff()].contents() && // same type of piece
		...
		// resolve if the piece is on same file of rank (if there are three same pieces then use file and rank)
		...
		{
			san->insert(1, string(ChessFileToChar(move.ff()) + string(ChessRankToChar(move.rf()))));
		...
		for(int ff = 0; ff < b.files(); ++ff)
		{
			...
			switch(b(rf, ff).contents())
			{
				...
				case ChessSquare::whitePawn:
					if (rf < 7 && b(rf + 1, ff).isEmpty())
					{
						if(rf == b.ranks() - 2) // promotion
						{
							...
							moves->add(ChessMove(rf, ff, rf + 1, ff, ChessMove::promoKing)); // some wild variants
					...
					for (s = -1; s <= 1; s += 2)
					{
						if (rf < 7 && ff + s >= 0 && ff + s <= 7 && b(rf + 1, ff + s).isBlack())
						{
							if(rf == b.ranks() - 2) // promotion
							{
								...
								moves->add(ChessMove(rf, ff, rf + 1, ff + s, ChessMove::promoKing)); // some wild variants
							} else
							{
						...
						if (rf == b.ranks() - 4)
						{
							if (ff + s >= 0 && ff + s <= 7 && (b.enPassant() == ff + s || b.enPassant() == Board::allCaptures) &&
								b(b.ranks() - 4, ff + s).contents() == ChessSquare::blackPawn && b(b.ranks() - 3, ff + s).isEmpty())
							{
								moves->add(ChessMove(rf, ff, b.ranks() - 3, ff + s, ChessMove::whiteEnPassant));
							}
						}
					}
					break;
				...
				case ChessSquare::blackPawn:
					...
					if (rf == b.ranks() - 2 && b(b.ranks() - 3 , ff).isEmpty() && b(b.ranks() - 4, ff).isEmpty())
					{
...
bool IsInCheck(Board b)
{
	...
	bool isFound = false;
	ChessSquare::E_contents target = b.toMove() == Board::white ? ChessSquare::whiteKing : ChessSquare::blackKing;
...
void GenLegalMoves(const Board& b, List<ChessMove>* moves)
{
		...
		for(i = 0; i < b.files(); ++i)
		{
			if(b(0, i) == ChessSquare::whiteKing)
			{
				...
				if(b.getCastle() & Board::whiteQS)
				{
					...
					if(!pieceInWay && b(0, 0) == ChessSquare::whiteRook &&
						i - 2 > 0)
					{
						if(!IsInCheck(b) && !WillBeInCheck(b, ChessMove(0, i, 0, i - 1))) // cannot castle through check, or when in check
							moves->add(ChessMove(0, i, 0, i - 2, ChessMove::whiteCastleQS));
					}
				}
			}
			//!!? break is possible here, unless two kings on rank and one can castle (some form of wild?)
		}
			...
			if(b(b.ranks() - 1, i) == ChessSquare::blackKing)
			{
				if(b.getCastle() & Board::blackKS)
				{
					...
					if(!pieceInWay && b(b.ranks() - 1, b.files() - 1) == ChessSquare::blackRook &&
						i + 2 < b.files())
					{
						if(!IsInCheck(b) && !WillBeInCheck(b, ChessMove(b.ranks() - 1, i, b.ranks() - 1, i + 1)))
						{
							moves->add(ChessMove(b.ranks() - 1, i, b.ranks() - 1, i + 2, ChessMove::blackCastleKS));
						}
					}
				}
				if(b.getCastle() & Board::blackQS)
				{
					...
					{
						if(!IsInCheck(b) && !WillBeInCheck(b, ChessMove(b.ranks() - 1, i, b.ranks() - 1, i - 1)))
							moves->add(ChessMove(b.ranks() - 1, i, b.ranks() - 1, i - 2, ChessMove::blackCastleQS));
					}
				}
			}
		}
	...
	while(i < moves->size())
	{
		if(WillBeInCheck(b, moves->operator[](i))) // could use a reference, but I like to keep my references const if I can w/o much trouble
			moves->remove(i);
		else
			++i;
	}
...
void MoveToAlgebraic(const ChessMove& move, const Board& b, string* out)
{
	...
	bool conflict = false, fileConflict = false, rankConflict = false; // use the file in case of a conflict, and rank if no file conflict, and both if necessary (e.g. there are three pieces accessing the same square)
//	cout << "here: " << b(move.rf, move.ff()).contents();
	switch(b(move.rf(), move.ff()).contents())
	{
		...
		case ChessSquare::blackPawn:
			...
			if (move.ff() != move.ft())
			{
				o << 'x' << ChessFileToChar(move.ft()) << ChessRankToChar(move.rt()); // Capture; use style "exd5"
			} else
			{
		...
		case ChessSquare::blackKing:
			// castling
			if (move.rf() == move.rt() &&
					move.rf() == ((b(move.rf(), move.ff()).isWhite()) ? 0 : b.ranks() - 1))
			{
		...
		default:
			...
			for(int i = 0; i < allMoves.size(); ++i)
			{
				if(allMoves[i] != move && // not the same move
					...
					b(move.rf(), move.ff()).contents() == b(allMoves[i].rf(), allMoves[i].ff()).contents()) // same type of piece
				{
					conflict = true;
...
// performs a lot of cleanup, so the move can be messy (calls RemoveChars("x+=#!? "))
bool AlgebraicToMove(const char constSAN[], const Board& b, ChessMove* move)
{
	...
	switch(san[0])
	{
		case 'N': case 'n': piece = b.isWhiteToMove() ? ChessSquare::whiteKnight : ChessSquare::blackKnight; break;
		case 'B': piece = b.isWhiteToMove() ? ChessSquare::whiteBishop : ChessSquare::blackBishop; break;
		case 'R': case 'r': piece = b.isWhiteToMove() ? ChessSquare::whiteRook : ChessSquare::blackRook; break;
		case 'Q': case 'q': piece = b.isWhiteToMove() ? ChessSquare::whiteQueen : ChessSquare::blackQueen; break;
		case 'K': case 'k': piece = b.isWhiteToMove() ? ChessSquare::whiteKing : ChessSquare::blackKing; break;
		case 'O': case 'o': // castling
			if(NumCharsInStr(san, 'O') > 2)
			{
				for(i = 0; i < allMoves.size(); ++i)
					if(allMoves[i].type() == ChessMove::whiteCastleQS || allMoves[i].type() == ChessMove::blackCastleQS)
					{
						assert(b.isWhiteToMove() && allMoves[i].type() == ChessMove::whiteCastleQS || b.isBlackToMove() && allMoves[i].type() == ChessMove::blackCastleQS);
						*move = allMoves[i];
						return true;
					}
				return false;
			...
			{
				for(i = 0; i < allMoves.size(); ++i)
					if(allMoves[i].type() == ChessMove::whiteCastleKS || allMoves[i].type() == ChessMove::blackCastleKS)
					{
						assert(b.isWhiteToMove() && allMoves[i].type() == ChessMove::whiteCastleKS || b.isBlackToMove() && allMoves[i].type() == ChessMove::blackCastleKS);
						*move = allMoves[i];
						return true;
					}
				return false;
	...
	{
		// is it a promotion?
		if(IsPromoChar(san[strlen(san) - 1])) //!!? lower case b is not counted as Bishop
		{
...
//			cout << "\n promotion";
		...
		// possibilities: f, f4, ef (e.p.), ef4, exf4 (e.p.), e3f4 (e.p.), e3xf4 (e.p.)
		// all x's have been removed, e.p. is taken care of
		if(!strlen(san))
			return false;
		...
		const size_t sanLength = strlen(san); // to avoid continually calling strlen()
		if(sanLength < 3)
			return false;
//		cout << "\n here: " << san;
...
const char gFENBeginNormalGame[] = "rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR w KQkq - 0 1";
