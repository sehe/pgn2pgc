...
public:
  ...
  ChessSquare(E_contents contents, E_status status = none)
      : fContents(contents), fStatus(status) {}

  bool isWhite() const {
    return fContents == whitePawn || fContents == whiteKnight ||
           fContents == whiteBishop || fContents == whiteRook ||
           fContents == whiteQueen || fContents == whiteKing;
  }
  bool isBlack() const {
    return fContents == blackPawn || fContents == blackKnight ||
           fContents == blackBishop || fContents == blackRook ||
           fContents == blackQueen || fContents == blackKing;
  }
  bool isEmpty() const { return fContents == empty; }
  bool isPawn() const {
    return fContents == blackPawn || fContents == whitePawn;
  }
...
public:
  ...
  bool processFEN(const std::string
                      &FENPosition); // true if valid position, false otherwise

  enum E_toMove { white, black, endOfGame };
  enum E_status { noStatus, checkmate, check, stalemate };
  enum E_castleAvailability {
    noCastle = 0,
    whiteKS = 1,
    whiteQS = 2,
    blackKS = 4,
  ...
  bool move(
      const ChessMove &); // private, need to remove calls to external functions
  bool processMove(const ChessMove &m); // make move and change status of game,
                                        // colorToMove etc
...
inline bool IsPromoChar(char c) {
  return toupper(c) == 'Q' || toupper(c) == 'N' || c == 'B' ||
         toupper(c) == 'R' || toupper(c) == 'K';
}
